...
namespace UrbanCompany.API.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public bool Make_Order(int id,IEnumerable<CartOrder> cart ,string date , string time, int convenience_fee)
        {
            ...
            {
                CustomerAddress = cart.Select(c=>c.Area).First();
        ...
        public bool AddToOrderOngoing(int order_id, IEnumerable<CartOrder> cart, IList<int> providers , string date, string time)
        {
            bool iscomplete = false;
            ...
            foreach (var c in cart)
            {
                ...
                og.ServiceDate = date;
                og.ServiceTime = time;
                og.DeliveryAddress = c.AddressLine + " , " + c.Area + " , " + c.city;
                og.Cost = c.Cost;
                og.Qty = c.Qty;
        ...
        {
            context.RemoveRange(orderOngoings);
            context.SaveChanges();
        }
    }
}