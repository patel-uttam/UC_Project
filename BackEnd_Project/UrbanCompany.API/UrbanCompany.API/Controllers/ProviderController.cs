...
namespace UrbanCompany.API.Controllers
{
    ...
    public class ProviderController : ControllerBase
    {
        ...
        // To fetch List of Provider base on category_name and city
        [HttpGet("{category}/{dist}/providers")]
        public IEnumerable<Provider> Get(string category, string dist)
        {
            return provider_repocsitory.GetProviderByCategoryName_City(category, dist);
        }
        ...
        [Authorize(Roles = Roles.Provider)]
        [HttpPut("{id}")]
        public Provider Put(int id, [FromBody] Provider provider)
        {
            if (provider.ProviderId == id)
            {
                return provider_repocsitory.UpdateProvider(provider);
            }
            else
            {
                return null;
